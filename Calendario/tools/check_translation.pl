...
if ( $infile eq "" ) {
  opendir ( DIR, "../translations" ) || die "se ha producido un error mientras se habría el fichero de traducción ../translations";
...
foreach $text ( sort { uc($a) cmp uc($b) } keys ( %text ) ) {
  if ( ! defined ( $trans{$text} ) ) {
    if ( ! $notfound ) {
      print "El texto siguiente no tiene una traducción en $infile:\n\n";
    }
    print "$text\n";
    $notfound++;
  }
...
foreach $text ( sort { uc($a) cmp uc($b) } keys ( %trans ) ) {
  if ( ! defined ( $text{$text} ) ) {
    if ( ! $extra ) {
      print "\nEl texto de traducción siguiente no se necesita en $infile:\n\n";
    }
    print "$text\n";
    $extra++;
  }
...
if ( ! $notfound ) {
  ...
  print "\n$notfound traducción desconocida.\n";
}
